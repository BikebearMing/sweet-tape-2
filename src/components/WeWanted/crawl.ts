...
export const WANTED = {
  ...
  /* THE PHONE'S ONE-SHOT REVEAL (see initFlat), in seconds rather than shares
     of a pin. STAGGER is between letters, POP is each box's own run; START is where on the screen each part sets off. */
  FLAT: {
    STAGGER: 0.018,
    POP: 0.6,
    START: "top 80%",
...
/* WHETHER THIS SHEET IS THE PHONE'S PLAIN SECTION — no pin, no crawl, the
   sentence as flat letters and the boxes in a grid. See initFlat. */
const flat = (root: HTMLElement): boolean =>
  Number.parseFloat(getComputedStyle(root).getPropertyValue("--wanted-grid")) >
  0;

/* THE PHONE: A ONE-SHOT REVEAL, played once when each part scrolls in rather
   than scrubbed. The letters come up out of their masks the way every other
   heading on the site does (REVEAL is the hero's), then each box pops as it
   scrolls in, with the desktop's own rise and overshoot. */
function initFlat(root: HTMLElement, boxes: HTMLElement[]): () => void {
  gsap.registerPlugin(ScrollTrigger);

  const chars = root.querySelectorAll<HTMLElement>(".wanted-plain .char");
  const { RISE, EASE } = WANTED.POP;
  const { STAGGER, POP, START } = WANTED.FLAT;

  const letters = gsap.fromTo(
    chars,
  ...
  /* EACH BOX ON ITS OWN TRIGGER — they stack a screen and more tall, so one
     trigger for all four would pop the last two where nobody is looking. */
  const pops = boxes.map((box) =>
    gsap.fromTo(
      box,
      { scale: 0, yPercent: RISE },
      { scale: 1, yPercent: 0, duration: POP, ease: EASE, paused: true },
    ),
  );

  const triggers = [
    ScrollTrigger.create({
      trigger: root.querySelector(".wanted-plain"),
      start: START,
      once: true,
      onEnter: () => letters.play(),
    }),
    ...boxes.map((box, i) =>
      ScrollTrigger.create({
        trigger: box,
  ...
  return () => {
    ...
    letters.kill();
    pops.forEach((t) => t.kill());
    gsap.set([...chars, ...boxes], { clearProps: "transform" });
  };
}
...
export function initWeWanted(root: HTMLElement): () => void {
  ...
  if (flat(root)) return initFlat(root, boxes);
  ...
  whenFace(() => {
    ...
    /* THE POPS. Scale from nothing while climbing into place, with one
         overshoot at the end of each — see POP. Not opacity: a box that fades
         is a box that was always there. */
    tl.fromTo(
      boxes,
      { scale: 0, yPercent: RISE },
      {
