...
export function initSuperPowersReveal(root: HTMLElement): () => void {
   ...
   * takes the fade off before the drop is declared, so the mark restarts from
   * the ceiling rather than fighting a fade on its way down. */
  ...
  /* And take the mark back off the card, which is what makes a card blank the
   * moment it stops being the one being read.
   *
   * "out" AND NOT NO ATTRIBUTE AT ALL, and the difference is a bug that made
   * every change of card look like the drawing had been switched off. Undeclaring
   * the drop outright returns the mark to the base rule's opacity 0 on a single
   * frame — and the transition that used to sit on .powers-mark-jump to catch
   * that fall could never fire, because a transition compares styles with
   * animation-derived values excluded and both sides of that comparison were the
   * base rule's 0. So the mark simply vanished mid-card, while the card it was on
   * was still shrinking. See the [data-mark="out"] rule in global.css, which
   * carries the exit as an animation of its own.
   *
