...
export function initArticleReveal(root: HTMLElement): () => void {
   ...
   *
   * THE CHIP IS PARKED HERE TOO, and that one is not a transform: it is held at
   * opacity 0, because a turn that begins from a standing chip is a chip that
   * jumps to edge-on and then turns. autoAlpha is an absolute value with no CSS
   * half to be added to, so it needs no `y: 0` of its own — only the same place
   * in the order. */
  const masked = [...title, ...deck];
  ...
  /* A headline flying in from nowhere and a chip turning over are exactly what
     the setting is asking about. What is wanted is all three blocks standing,
     so the parks above are handed straight back — under a live attribute the
